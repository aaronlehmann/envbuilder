...
func TestCompile(t *testing.T) {
	...
	t.Run("UnknownOption", func(t *testing.T) {
		t.Parallel()
		spec := &features.Spec{}
		_, err := spec.Compile("", false, "containerUser", "remoteUser", map[string]any{
			"unknown": "value",
		})
	...
	t.Run("Basic", func(t *testing.T) {
		...
		directive, err := spec.Compile("", false, "containerUser", "remoteUser", nil)
		require.NoError(t, err)
	...
	t.Run("Mount", func(t *testing.T) {
		...
		directive, err := spec.Compile("/scratch", true, "containerUser", "remoteUser", nil)
		require.NoError(t, err)
		require.Equal(t, "WORKDIR /\nRUN --mount=type=bind,source=/scratch,target=/scratch,rw _CONTAINER_USER=\"containerUser\" _REMOTE_USER=\"remoteUser\" ./install.sh", strings.TrimSpace(directive))
	})
}
